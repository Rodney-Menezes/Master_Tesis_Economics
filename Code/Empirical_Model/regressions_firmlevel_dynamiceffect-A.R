...
# Crea rezagos (por defecto, un periodo) de los controles macro dentro de país
add_lagged_macro_controls <- function(df, vars, lag = 1) {
  vars_present <- intersect(vars, names(df))
  if (length(vars_present) == 0) {
    return(df)
  }
...
macro_controls <- c("dlog_gdp", "dlog_cpi", "unemp", "embigl")
df <- df %>%
  add_lagged_macro_controls(macro_controls, lag = 1)
lagged_macro_controls <- paste0("L1_", macro_controls)
...
controls_macro  <- intersect(lagged_macro_controls, names(df_dyn_nocy))
...
# --------------------------------------------------------
controls_firm  <- c("L1_rsales_g_win", "L1_size_raw", "L1_current_ratio_win")
controls_macro <- intersect(lagged_macro_controls, names(df_dyn))
...
# --------------------------------------
controls_firm  <- c("L1_rsales_g_win", "L1_size_raw", "L1_current_ratio_win")
controls_macro <- intersect(lagged_macro_controls, names(df_dyn11))
...
controls_macro <- lagged_macro_controls
...
controls_macro13 <- intersect(lagged_macro_controls, names(df_dyn13))
