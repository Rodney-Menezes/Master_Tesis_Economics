...
# --------------------------------------------------------
# Factor cíclico rezagado por país a partir de variables macro
# --------------------------------------------------------
macro_series <- intersect(c("dlog_gdp", "cpi", "embi"), names(df))

if (length(macro_series) > 0) {
  df <- df %>%
    dplyr::group_by(Country) %>%
    dplyr::arrange(dateq, .by_group = TRUE)

  if ("dlog_gdp" %in% macro_series) {
    df <- df %>%
      dplyr::mutate(L1_dlog_gdp = dplyr::lag(dlog_gdp, 1))
  }

  if ("cpi" %in% macro_series) {
    df <- df %>%
      dplyr::mutate(
        dcpi     = cpi - dplyr::lag(cpi),
        L1_dcpi  = dplyr::lag(dcpi, 1)
      )
  }

  if ("embi" %in% macro_series) {
    df <- df %>%
      dplyr::mutate(L1_embi = dplyr::lag(embi, 1))
  }

  cyc_columns <- intersect(c("L1_dlog_gdp", "L1_dcpi", "L1_embi"), names(df))

  if (length(cyc_columns) > 0) {
    df <- df %>%
      dplyr::group_modify(~ {
        tmp <- .x
        available_cols <- cyc_columns[sapply(cyc_columns, function(var) any(!is.na(tmp[[var]])))]

        if (length(available_cols) == 0) {
          tmp$cyc_factor_L1 <- NA_real_
          return(tmp)
        }

        scaled_list <- lapply(available_cols, function(var) {
          x <- tmp[[var]]
        ...
        names(scaled_list) <- available_cols
        valid_cols <- vapply(scaled_list, function(x) any(!is.na(x)), logical(1))
        scaled_list <- scaled_list[valid_cols]
        ...
        scaled_df <- tibble::as_tibble(scaled_list)
        complete_rows <- stats::complete.cases(scaled_df)
        cyc_scores <- rep(NA_real_, nrow(scaled_df))

        if (sum(complete_rows) >= 2) {
          pca_res <- stats::prcomp(
            scaled_df[complete_rows, , drop = FALSE],
            center = FALSE,
            scale. = FALSE
          )

          cyc_scores[complete_rows] <- pca_res$x[, 1]
          mean_cyc <- mean(cyc_scores[complete_rows], na.rm = TRUE)
          sd_cyc   <- stats::sd(cyc_scores[complete_rows], na.rm = TRUE)

          if (!is.na(sd_cyc) && sd_cyc > 0) {
            cyc_scores <- (cyc_scores - mean_cyc) / sd_cyc
          } else {
            cyc_scores[] <- NA_real_
          }
        }

        tmp$cyc_factor_L1 <- cyc_scores
        tmp
      }) %>%
      dplyr::ungroup()
  } else {
  ...
  if ("cyc_factor_L1" %in% names(df)) {
    df <- df %>%
      dplyr::mutate(
        lev_pcf = L1_lev_dm * cyc_factor_L1,
        dd_pcf  = L1_dd_dm  * cyc_factor_L1
      )
  }

  df <- df %>%
    dplyr::select(-tidyr::any_of("dcpi"))
} else {
  df <- df %>%
    dplyr::ungroup()
}
...
res_lev_nocy <- map(0:12, function(h) {
  rhs_terms <- c("lev_shock", controls_vec_nocy)
  feols(
    as.formula(paste0(
      "cumF", h, "_dlog_capital ~ ", paste(rhs_terms, collapse = " + "),
      " | name + sec + dateq"
    )),
    data    = df_dyn_nocy,
...
res_dd_nocy <- map(0:12, function(h) {
  rhs_terms <- c("d2d_shock", controls_vec_nocy)
  feols(
    as.formula(paste0(
      "cumF", h, "_dlog_capital ~ ", paste(rhs_terms, collapse = " + "),
      " | name + sec + dateq"
...
controls_macro  <- intersect(c("lev_pcf", "dd_pcf"), names(df_dyn))
controls_common <- c(controls_firm, controls_macro)

# 5) Estimar respuesta promedio al shock (horizonte 0–12)
# --------------------------------------------------------
res_avg <- map(0:12, function(h) {
  dep_var <- paste0("cumF", h, "_dlog_capital")
  rhs_terms <- c("shock_exp", "lev_shock", "d2d_shock", controls_common)
...
# 3) Interacciones con el shock monetario
# ---------------------------------------
df <- df %>%
  mutate(
    lev_shock = L1_lev_dm * shock_exp,
...
# --------------------------------------
controls_firm   <- c("L1_rsales_g_win", "L1_size_raw", "L1_current_ratio_win")
controls_macro  <- intersect(c("lev_pcf", "dd_pcf"), names(df_dyn11))
...
res_avg <- map(0:12, function(h) {
  dep_var <- paste0("cumF", h, "_dlog_capital")
  rhs_terms <- c("shock_exp", "lev_shock", "d2d_shock", "Ldl_capital", controls_common)
  fml <- as.formula(paste0(
    dep_var,
    " ~ ", paste(rhs_terms, collapse = " + "),
    " | name + sec + dateq"
...
if ("cyc_factor_L1" %in% names(df_cntl)) {
  df_cntl <- df_cntl %>%
    dplyr::mutate(
      lev_pcf = L1_lev_dm * cyc_factor_L1,
      dd_pcf  = L1_dd_dm  * cyc_factor_L1
...
if ("cyc_factor_L1" %in% names(df_cntl13)) {
  df_cntl13 <- df_cntl13 %>%
    dplyr::mutate(
      lev_pcf = L1_lev_dm * cyc_factor_L1,
      dd_pcf  = L1_dd_dm  * cyc_factor_L1
...
# 2) Crear interacciones con el shock monetario
# -------------------------------------------
df_cntl13 <- df_cntl13 %>%
  mutate(
...
controls_macro13 <- intersect(c("lev_pcf", "dd_pcf"), names(df_dyn13))

# 4a) Estimar dinámica Size + Leverage
res_lev_size <- map(0:12, function(h) {
  rhs_terms <- c("size_shock", "lev_shock", controls_firm13, controls_macro13)
  feols(
    as.formula(paste0(
      vars13[h+1],
      " ~ ", paste(rhs_terms, collapse = " + "),
...
res_dd_size <- map(0:12, function(h) {
  rhs_terms <- c("size_shock", "d2d_shock", controls_firm13, controls_macro13)
  feols(
    as.formula(paste0(
      vars13[h+1],
      " ~ ", paste(rhs_terms, collapse = " + "),
