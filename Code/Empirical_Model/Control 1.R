...
res_lev_nocy <- map(0:12, function(h) {
  feols(
    as.formula(paste0(
      "cumF", h, "_dlog_capital ~ ", rhs_lev_nocy,
...
res_dd_nocy <- map(0:12, function(h) {
  feols(
    as.formula(paste0(
      "cumF", h, "_dlog_capital ~ ", rhs_dd_nocy,
      " | name + sec + Country^dateq"
    )),
    data    = df_dyn_nocy,
    cluster = ~ Country + dateq + name
...
res_dd_lag <- map(0:12, function(h) {
  ...
  ffml_str   <- paste(dep_var, "~", paste(rhs_terms, collapse = " + "), "| name + sec + Country^dateq")
  feols(as.formula(fml_str), data = df_dyn11, cluster = ~ Country + dateq + name)
})
...
controls_firm <- c("L1_rsales_g_win", "L1_size_raw", "L1_current_ratio_win")
controls_macro <- intersect(c("lev_gdp", "dd_gdp"), names(df_dyn))
rhs_terms_avg <- paste(c("shock_exp", "lev_shock", "d2d_shock", "Ldl_capital", controls_firm, controls_macro),
                       collapse = " + ")
...
controls_macro13  <- intersect(c("lev_gdp", "dd_gdp"), names(df_dyn13))
controls_vec13    <- c(controls_firm13, controls_macro13)
rhs_lev_size      <- paste(c("size_shock", "lev_shock", "lev_shock_gdp", controls_vec13), collapse = " + ")
