function objective = transition_path_decisionObjectiveFunction(x,iProd,iCash,vDefaultCutoff_prime,...
...
vValuePrime         = reshape(interpn(mProdGrid,mCashGrid,mValue_prime,mShocksGrid(:,1),vCashPrime),nShocks,1);

% Integrate over exit shock
vValuePrime         = ppiExit * (1 - (vCashPrime <= zeros(nShocks,1))) .* vCashPrime + ...
                        (1 - ppiExit) * (1 - (vCashPrime <= reshape(repmat(vDefaultCutoff_prime,[1 nOmega]),nShocks,1))) .* vValuePrime;
...
objective       = -(mCashGrid(iProd,iCash) - q * kPrime + debtPrice * bPrime + sdf * expectedValue);
